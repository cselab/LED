...
hyper_params_dict["RNN_cell_type"]              = ["mlp"]
hyper_params_dict["RNN_activation_str"]         = ["celu"]
hyper_params_dict["RNN_layers_num"]             = [3]
hyper_params_dict["RNN_layers_size"]            = [100]
...
# # |    122 | 1.90578 |  0.55086  | 1.05572  | GPU-ARNN-scaler_MinMaxZeroOne-OPT_adabelief-LR_0.001-NL_0.0-L2_0.0-RS_10-CHANNELS_1-16-32-64-8-KERNELS_5-5-5-5-BN_0-TRANS_1-POOL_avg-ACT_celu-DKP_1.0-LD_8-PRETRAIN-AE_1-RS_7-C_mlp-R_3x100-SL_25-LFO_1-LFL_1                |
...
# hyper_params_dictionary_list[0]["test_on_test"]     = 0
...
if not IS_MULTISCALE_TESTING:
    for run_num in range(num_runs):
        with open("./Tasks/P4_B{:}_greasy_AE_MLP_tasks.txt".format(run_num + 1),
                  "w") as file:
    ...
    for run_num in range(num_runs):
        with open("./Tasks/P4T_B{:}_greasy_AE_MLP_tasks.txt".format(run_num + 1),
                  "w") as file:
            for dict_num in range(run_num * max_jobs_per_run,
                                  (run_num + 1) * max_jobs_per_run):
                hyper_param_dict_case = hyper_params_dictionary_list[dict_num]
