...
class KS:
    ...
    def setup_fourier(self, coeffs=None):
        ...
        self.k = np.r_[0:self.N / 2, 0,
                       -self.N / 2 + 1:0] / self.L  # Wave numbers
    ...
    def setup_etdrk4(self):
        ...
        self.r = np.exp(1j * pi * (np.r_[1:self.M + 1] - 0.5) /
                        self.M)  # roots of unity
        self.LR = self.dt * np.repeat(
            self.l[:, np.newaxis], self.M, axis=1) + np.repeat(
                self.r[np.newaxis, :], self.N, axis=0)
        self.Q = self.dt * np.real(
            np.mean((np.exp(self.LR / 2.) - 1.) / self.LR, 1))
        self.f1 = self.dt * np.real(
            np.mean((-4. - self.LR + np.exp(self.LR) *
                     (4. - 3. * self.LR + self.LR**2)) / (self.LR**3), 1))
        self.f2 = self.dt * np.real(
            np.mean((2. + self.LR + np.exp(self.LR) * (-2. + self.LR)) /
                    (self.LR**3), 1))
        self.f3 = self.dt * np.real(
            np.mean((-4. - 3. * self.LR - self.LR**2 + np.exp(self.LR) *
                     (4. - self.LR)) / (self.LR**3), 1))
        self.g = -0.5j * self.k
        ...
        if (v0 is None):
            ...
            if (u0 is None):
                ...
                if testing:
                    ...
                    u0 = np.cos(
                        self.x / self.L) * (1. + np.sin(self.x / self.L))
                else:
                    # random noise
    ...
    def step(self):
        ...
        #
        self.v = self.E * v + Nv * self.f1 + 2. * (Na +
                                                   Nb) * self.f2 + Nc * self.f3
        self.stepnum += 1
        self.t += self.dt
        ...
        if (correction == []):
            for n in range(1, self.nsteps + 1):
                try:
                    ...
                    self.vv.resize((self.nout + 1,
                                    self.N))  # nout+1 because the IC is in [0]
                    self.tt.resize(self.nout +
                                   1)  # nout+1 because the IC is in [0]
                    return -1
    ...
    def compute_Ek(self):
        ...
        self.Ek_k = np.sum(self.Ek_kt, 0) / (
            self.ioutnum + 1
        )  # not self.nout because we might not be at the end; ioutnum+1 because the IC is in [0]
        ...
        self.Ek_ktt = np.cumsum(self.Ek_kt, 0) / np.arange(
            1, self.ioutnum + 2
        )[:,
        ...
        self.Ek_tt = np.cumsum(self.Ek_t, 0) / np.arange(
            1, self.ioutnum + 2
        )[:,
          None]  # not self.nout because we might not be at the end; ioutnum+1 because the IC is in [0] +1 more because we divide starting from 1, not zero
    ...
    def compute_Ek_kt(self):
        try:
            self.Ek_kt = 1. / 2. * np.real(
                self.vv.conj() * self.vv / self.N) * self.dx
        except FloatingPointError:
            #
            ...
            while problem:
                try:
                    self.Ek_kt[0:self.nout + 1 -
                               remove, :] = 1. / 2. * np.real(
                                   self.vv[0:self.nout + 1 - remove].conj() *
                                   self.vv[0:self.nout + 1 - remove] /
                                   self.N) * self.dx
                    problem = False
                except FloatingPointError:
                    remove += 1
    ...
    def space_filter(self, k_cut=2):
        ...
        for n in range(self.nout + 1):
            ...
            v_filt[
                np.abs(self.k) >= k_cut] = 0  # set to zero wavenumbers > k_cut
