...
class ConvRNNCell(nn.Module):
    ...
    def __init__(self,
        ...
        if self.rnn_cell_type == "lstm":
            ...
            self.Bci = Parameter(
                self.torch_dtype(1, hidden_channels, shape[0], shape[1]))
            self.Bcf = Parameter(
                self.torch_dtype(1, hidden_channels, shape[0], shape[1]))
            self.Bco = Parameter(
                self.torch_dtype(1, hidden_channels, shape[0], shape[1]))
            ...
            self.Wiz = nn.Conv2d(self.input_channels,
                                 self.hidden_channels,
                                 self.kernel_size,
                                 stride=1,
                                 padding=self.padding,
                                 bias=True)
            self.Whz = nn.Conv2d(self.hidden_channels,
                                 self.hidden_channels,
                                 self.kernel_size,
                                 stride=1,
                                 padding=self.padding,
            ...
            self.Wir = nn.Conv2d(self.input_channels,
                                 self.hidden_channels,
                                 self.kernel_size,
                                 stride=1,
                                 padding=self.padding,
                                 bias=True)
            self.Whr = nn.Conv2d(self.hidden_channels,
                                 self.hidden_channels,
                                 self.kernel_size,
                                 stride=1,
                                 padding=self.padding,
            ...
            self.Wih = nn.Conv2d(self.input_channels,
                                 self.hidden_channels,
                                 self.kernel_size,
                                 stride=1,
                                 padding=self.padding,
                                 bias=True)
            self.Whh = nn.Conv2d(self.hidden_channels,
                                 self.hidden_channels,
                                 self.kernel_size,
                                 stride=1,
                                 padding=self.padding,
